# DEMO-RUNBOOK: "Quantum programming in ordinary Julia, assisted by LLM agents" (JuliaCon 2026, 15 min)
...
1. **Rebuild the demo environment** (dual-develop: Sturm's own project
   ...
   Pkg.precompile()'
   ```
   Run this from `/home/tobias/Projects/Sturm.jl`. Do this even if
...
2. **Orkan lib check**, confirm the shared library the demo will
   ...
   ```bash
   ls -la /home/tobias/Projects/orkan/cmake-build-release/src/liborkan.so
   ```
...
3. **Rebuild and smoke-test the deck.** From `talks/juliacon-2026/`:
   ...
   - Normal: `file:///home/tobias/Projects/Sturm.jl/talks/juliacon-2026/talk.html`.
     Click through all 19 slides (s0, s0a, s0b, s0c, s1–s15) once with →, confirm
     every build step advances and the s0b chimera and s9 Bennett
     construction (the two SMIL-animated figures in the deck) render without console errors,
...
1. Open a terminal, position it (§a.5), set env vars **before** starting
   ...
   cd /home/tobias/Projects/Sturm.jl
...
because counts drift as soon as you run anything (§b.3).
