...
| 7 | **`classicalise`** (channel → column-stochastic matrix; v0.1 §7.1 "Noise") | **(c) needs re-derivation — spec ruled (S31/T2, 2026-07-25), gated on M11** | The seventh, **unlogged** carried contract, discovered by the M11 design round (audit item V10): zero hits in this plan or PRD-v2 before this row; its only source is `Sturm-PRD.md:457`, and the defect is in the contract **text itself** — *"`classicalise(f)` returns 2×2 column-stochastic matrix"* is a silent single-qubit assumption in the spec, not just the code. **Not v2-safe as carried.** The re-derivation is ruled but not yet normative: S31 keeps the v0.1 name for the v0.1 meaning — `classicalise(g::ChannelDAG \| ::ChannelValue \| ::LogicalChannel) -> Matrix{Float64}`, the decohered channel `Δ_out ∘ 𝓔 ∘ Δ_in`, **arity taken from the ports** (the v0.1 defect cannot recur), exact by replay, loud above `CLASSICALISE_MAXWIRES`, docstring-flagged **PHASE-BLIND — never a channel-equivalence test** (the F3-barred criterion; named test `M11.CLASSICALISE.IS-PHASE-BLIND` asserts `classicalise(id) == classicalise(Ad_Z)` *deliberately*) — and token-record introspection is a **different operation with a different name**, `record_distribution(::ClassicalToken)` (merging them is the sample/record/assert conflation §3.6 exists to prevent). Verdict flips to (a) when M11 lands both in PRD-v2 normative text and code (bead `qmpo`). |

**Verdict counts: (a) re-derived = 5 · (b) verbatim = 1 · (c) needs
re-derivation = 1.** The single (c) is `classicalise` — logged here
**before** the milestone that consumes it, which is the table's whole point
(F8 was catchable precisely because the contract was visible ahead of its
consumer). *(History: 4/1/1 until 2026-07-25, the (c) being QECC-as-HOF;
ruling T1 closed that in the PRD → 5/1/0; ruling T2 then logged
`classicalise`, the seventh contract V10 discovered, → 5/1/1 on
2026-08-04.)* The audit of the six contracts F31 enumerated remains
