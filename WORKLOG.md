...
|---|---|---|---|
| 89 | [sessions-89.md](worklog/sessions-89.md) | 2026-05-05 | **4ceh decomposed into TWO independent bugs** (`l5s5` + `grq5`): (A) `qsvt_phases` analytic→reflection conversion broken — confirmed via §2.1 derivation from Laneve PDF + 8-permutation sign sweep + PennyLane `qml.transform_angles` formula cross-check; (B) `qsvt_reflect!` single-qubit Rz ≠ multi-ancilla `Π^φ` for n_anc≥2 — confirmed via hand-validated deg-3 phases isolating B from A; **reverses session 88's "do not repeat multi-controlled-reflection swap" guidance** (was confounded by Bug A). Tooling: `test/test_qsvt_amplitude_level.jl` deterministic amplitude probe; `docs/physics/laneve_2025_gqsp_nlft.md` extended with §2.1 chain + §4.3 Q→P swap |
